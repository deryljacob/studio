/**
 * @fileOverview API Route for handling Cloud Tasks for the Stuck Blockage Check.
 *
 * This endpoint is designed to be called exclusively by Google Cloud Tasks.
 * It verifies the authenticity of the incoming request and then triggers the
 * core logic to check if a previously cancelled event's calendar block has
 * been correctly removed.
 */

import { NextRequest, NextResponse } from 'next/server';
import { StuckBlockageTaskPayload } from '@/lib/types';
import { runStuckBlockageCheck } from '@/lib/services/run-stuck-blockage-check';
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const payload = (await request.json()) as StuckBlockageTaskPayload;

        if (!payload.userId || !payload.cancelledBookingCode || !payload.checkinDate) {
             return NextResponse.json({ error: 'Invalid task payload' }, { status: 400 });
        }

        // 3. Execute the core logic
        await runStuckBlockageCheck(payload);
        
        // 4. Acknowledge the task was successfully processed
        return NextResponse.json({ status: 'success' }, { status: 200 });

    } catch (error) {
        console.error('[STUCK_BLOCKAGE_HANDLER_ERROR]', error);
