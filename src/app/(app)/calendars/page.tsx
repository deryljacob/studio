...
import { isSameDay, fromUnixTime, getDay, startOfDay } from "date-fns";
...
function simpleIcsParser(icalData: string, calendarId: string, userId: string): EventType[] {
    ...
    while ((match = eventRegex.exec(icalData)) !== null) {
        ...
        if (uidMatch && dtstartMatch && dtendMatch) {
            ...
            // This function parses a date string from an iCal file.
            // It can handle both date-only (e.g., "20240725") and date-time (e.g., "20240725T100000Z") formats.
            // All dates are parsed into UTC to ensure consistency and avoid timezone-related bugs.
            ...
            // For all-day events, the iCal standard specifies the end date is often exclusive.
            // For example, an event ending on the 25th might have an end date of the 26th.
            // To make the event inclusive for rendering, we subtract one day.
            // This does not apply to timed events, which are usually precise.
...
export default function CalendarsPage() {
  ...
  const getNextAvailableColor = () => {
    const usedColors = new Set(calendarSources.map(source => source.color));
    const availableColor = CHART_COLORS.find(color => !usedColors.has(color));
    
    // If all predefined colors are used, fallback to modulo logic
    return availableColor || `hsl(var(--chart-${(calendarSources.length % 5) + 1}))`;
  };
  ...
  const handleSyncSingle = async (calendarId: string, feedUrl: string, options: { silent?: boolean } = {}) => {
    ...
    try {
        // 1. Get the user's ID token for server-side verification.
        const idToken = await user.getIdToken();

        // 2. Fetch iCal data via server action proxy, passing the token.
        const result = await fetchIcal({ url: feedUrl, idToken });
